---
name: 规格驱动开发
description: 编码前先写规格。启动新项目、新功能或重大变更且尚无规格时使用；需求不清晰、有歧义或只有模糊想法时使用；单个需求横跨多个可独立测试的能力、需要先拆成模块能力图时使用。
---
...
Execute tasks one at a time following `skills/增量实现/SKILL.md` (`incremental-implementation`) and `skills/tdd开发/SKILL.md` (`test-driven-development`). Use `skills/上下文工程/SKILL.md` (`context-engineering`) to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
