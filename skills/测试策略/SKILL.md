...
description: 产出真正信心的测试策略：测试金字塔、覆盖率规则、测什么与不测什么。设计测试套件、决定测试类型或设定覆盖率目标时使用。
---
...
For event-driven systems, use JSON Schema or Avro schema registry. Schema evolution rules: additive changes (new optional field) are backward-compatible, removals are breaking. Cross-reference: `事件驱动架构/SKILL.md`.
...
Cross-reference: `错误处理与韧性/SKILL.md` for resilience patterns, `混沌工程/SKILL.md` for production chaos testing.
...
Cross-reference: `事件驱动架构/SKILL.md` for event design and delivery guarantees.
...
*Internal references*: `测试实现/SKILL.md`, `性能测试/SKILL.md`, `安全测试/SKILL.md`, `错误处理与韧性/SKILL.md`, `事件驱动架构/SKILL.md`, `混沌工程/SKILL.md`
