...
description: Schema 设计、多租户数据隔离与迁移管理：Firestore 与 PostgreSQL 模式、RLS、UUID v7 约定。设计数据库表结构、编写迁移或实现多租户数据访问时使用。
---
...
For reliable event publishing alongside database writes (see `事件驱动架构/SKILL.md`):
...
*Internal references*: `合规与隐私/SKILL.md`, `事件驱动架构/SKILL.md`, `认证与授权/SKILL.md`
