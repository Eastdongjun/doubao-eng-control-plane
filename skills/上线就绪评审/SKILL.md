...
description: 生产就绪 GO/NO-GO 评审框架。覆盖可用性、可扩展性、可观测性、安全、合规的 NFR 检查表。部署新服务或重大变更到生产前使用。
---
...
- [ ] Structured logging with correlation ID (see `可观测性/SKILL.md`)
...
- [ ] Authentication and authorization enforced on all endpoints (see `认证与授权/SKILL.md`)
- [ ] Tenant isolation verified with automated tests
- [ ] Secrets management validated (no hardcoded credentials)
- [ ] Security scanning passed: SAST, dependency scan, container scan (see `安全测试/SKILL.md`)
...
- [ ] GDPR compliance verified (if handling EU data) — see `合规与隐私/SKILL.md`
- [ ] Multi-tenant data isolation verified

### 7. Release Safety

- [ ] Rollback strategy defined and tested (see `发布管理/SKILL.md`)
...
- [ ] Load test executed at 2x expected peak (see `性能测试/SKILL.md`)
...
- [ ] Monthly cost estimate documented (see `云成本治理/SKILL.md`)
...
*Internal references*: `可观测性/SKILL.md`, `安全设计/SKILL.md`, `安全测试/SKILL.md`, `合规与隐私/SKILL.md`, `发布管理/SKILL.md`, `cicd流水线/SKILL.md`
