...
description: 带自动化语义化版本（SemVer）、变更日志生成、发布说明、回滚策略与热修复流程的发布管理。配置发布、写变更日志、规划回滚或处理热修复时使用。
---
...
Merge to main → CI verifies (see `质量门禁/SKILL.md`) → tool analyzes commits → bumps version in package.json → generates CHANGELOG.md → creates git tag → creates GitHub Release with release notes.
...
*Internal references*: `cicd流水线/SKILL.md`, `功能开关/SKILL.md`, `质量门禁/SKILL.md`, `架构决策记录/SKILL.md`
