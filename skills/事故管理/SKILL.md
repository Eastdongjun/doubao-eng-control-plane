...
description: 从发现到复盘的事故响应流程：严重级别、沟通模板、无责复盘、事故指标。定义值班流程、响应生产事故或开展复盘时使用。
---
...
Runbooks for every known failure scenario (see `可观测性/SKILL.md`). Written for someone at 3 AM who doesn't know the service deeply. Copy-pasteable commands, clear escalation triggers.
...
*Internal references*: `可观测性/SKILL.md`, `上线就绪评审/SKILL.md`, `发布管理/SKILL.md`
