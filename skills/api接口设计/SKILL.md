...
description: REST 和 GraphQL 的 API 设计规范：资源命名、版本控制、分页、错误响应（RFC 7807）、OpenAPI 优先工作流、向后兼容。设计接口、编写接口处理逻辑或定义接口契约时使用。
---
...
The same principle applies to event schemas. Use JSON Schema diff or Avro compatibility checks in CI. Cross-reference: `事件驱动架构/SKILL.md`.
...
Cross-reference: `事件驱动架构/SKILL.md` for event design patterns and delivery guarantees.
...
Cross-reference: `可观测性/SKILL.md` for SLO-based alerting, `性能测试/SKILL.md` for load testing with k6.
...
*Internal references*: `认证与授权/SKILL.md`, `测试实现/SKILL.md`, `技术文档/SKILL.md`, `安全设计/SKILL.md`, `事件驱动架构/SKILL.md`, `可观测性/SKILL.md`, `性能测试/SKILL.md`
