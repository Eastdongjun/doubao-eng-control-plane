...
description: 把安全作为设计属性而非后补层：OWASP Top 10、供应链安全、密钥管理、零信任。编写接口、处理用户输入、管理依赖或配置基础设施时使用。
---
...
**Broken Authentication**: robust session management (see `认证与授权/SKILL.md`). Firebase ID tokens: 1hr (not configurable), use `checkRevoked` for sensitive ops. Custom auth: 15min access, 7d refresh. Rate limiting on login endpoints. No passwords in logs.
...
**Security Logging and Monitoring Failures**: log security-relevant events (login attempts, permission denials, data access). Ensure logs are tamper-resistant and retained per compliance requirements (see `合规与隐私/SKILL.md`).
...
Cross-reference: `默认漏洞检测/SKILL.md` for automated detection of fail-open defaults, `危险边缘识别/SKILL.md` for footgun design patterns.
...
*Internal references*: `认证与授权/SKILL.md`, `合规与隐私/SKILL.md`, `安全测试/SKILL.md`, `容器化/SKILL.md`
