...
name: 架构决策记录
cluster: foundations
description: 架构决策记录（ADR）的治理与格式：ADR 生命周期、评审流程、何时该写 ADR。做架构决策、引入新技术或改变集成模式时使用。
---
...
**Cost impact is mandatory.** Every ADR must include estimated cost implications (infrastructure, development time, operational complexity). Reference `云成本治理/SKILL.md` for cost modeling.
...
4. Decisional review meeting: 30 min max (see `架构沟通/SKILL.md`)
...
- GCP Free Tier analysis: see 云成本治理/SKILL.md
...
*Internal references*: `架构沟通/SKILL.md`, `架构绘图/SKILL.md`, `云成本治理/SKILL.md`, `合规与隐私/SKILL.md`
