...
| Skill | Relationship |
|-------|-------------|
| [threat-detection](../威胁检测/SKILL.md) | Anomaly detection in LLM inference API logs can surface model inversion attacks and systematic prompt injection probing |
| [incident-response](../事故响应/SKILL.md) | Confirmed prompt injection exploitation or data extraction from a model should be classified as a security incident |
| [cloud-security](../云安全/SKILL.md) | LLM API keys and model endpoints are cloud resources — IAM misconfiguration enables unauthorized model access (AML.T0012) |
| [security-pen-testing](../渗透测试/SKILL.md) | Application-layer security testing covers the web interface and API layer; ai-security covers the model and agent layer |
