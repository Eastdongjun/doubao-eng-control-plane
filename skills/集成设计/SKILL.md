...
description: 跨边界通信的集成设计：模式目录、契约优先设计、防腐层、跨边界数据一致性。设计服务集成、定义契约或规划遗留系统集成时使用。
---
...
**Saga**: use when steps must execute in order and each step has a defined compensating action. See `事件驱动架构/SKILL.md` for orchestration vs choreography decision.
...
*Internal references*: `功能分析/SKILL.md`, `api接口设计/SKILL.md`, `事件驱动架构/SKILL.md`, `微服务模式/SKILL.md`, `错误处理与韧性/SKILL.md`, `遗留系统改造/SKILL.md`
