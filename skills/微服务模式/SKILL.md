...
description: 服务拆分、服务间通信与运维关注的微服务模式：限界上下文、每服务一库、CQRS、API 网关、分布式追踪、韧性。设计服务边界、选择通信模式或运维分布式系统时使用。
---
...
**REST** for public APIs and simple internal queries. Follow conventions from `api接口设计/SKILL.md`.
...
**Events** (notification of something that happened): use for decoupled communication where the producer does not care who consumes the event. See `事件驱动架构/SKILL.md` for CloudEvents format, outbox pattern, and saga patterns.
...
For detailed instrumentation patterns, see `可观测性/SKILL.md`.
...
*Internal references*: `事件驱动架构/SKILL.md`, `api接口设计/SKILL.md`, `错误处理与韧性/SKILL.md`, `可观测性/SKILL.md`
